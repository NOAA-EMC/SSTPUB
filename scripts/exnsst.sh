...
#
  ...
  ln -s $SALCLM    salclm
#
# input FV3GFS 0-hour surface forecast file as the analysis currently at Gaussian grids, for T1534, 3072 x 1536
...
#
  ...
  do
     if [ $SFCIO = "nemsio" ] ; then
        ...
        sfcanl_name=model/atmos/history/gdas.t{$cycle}z.sfc.f000.nc
     else
       echo "invalid sfcio"
       exit
     fi
     ln -fs $sfcanl_name sfcanl
...
#
  ...
  do
     if [ $SFCIO = "nemsio" ] ; then
        ...
        iceanl_name=obs/$CDUMP.t{$cycle}z.seaice.5min.blend.grb
     else
       echo "invalid sfcio for iceanl"
       exit
     fi
     ln -fs $iceanl_name iceanl
